...
export const findAll = async (page: number = 1, limit: number = 50): Promise<any[]> => {
  ...
  const [rows]: any = await db.query('SELECT * FROM periodo_facturacion WHERE deleted_at IS NULL ORDER BY fecha_inicio DESC LIMIT ? OFFSET ?', [limit, offset]);
  return rows;
};

export const findByMesAnio = async (mes_anio: string): Promise<any> => {
  const [rows]: any = await db.query('SELECT id FROM periodo_facturacion WHERE mes_anio = ? AND deleted_at IS NULL', [mes_anio]);
  return rows[0] || null;
};

export const create = async (periodo: any): Promise<number> => {
...
export const update = async (id: number, periodo: any): Promise<number> => {
  const { mes_anio, factor_multiplicador, tarifa_kwh, tarifa_kwh_punta, tarifa_mantenimiento_normal, tarifa_mantenimiento_tiempo_real, fecha_inicio, fecha_fin } = periodo;
  const [result]: any = await db.query(
