...
export const findActivosPorPeriodo = async (periodo_id: any) => {
...
export const create = async (cargo: any, periodosIds: any[]) => {
...
export const update = async (id: any, cargo: any, periodosIds: any[]) => {
  const connection = await db.getConnection();
  await connection.beginTransaction();
...
export const softDelete = async (id: any) => {
  const [result]: any = await db.query('UPDATE catalogo_cargo SET deleted_at = CURRENT_TIMESTAMP WHERE id = ?', [id]);
