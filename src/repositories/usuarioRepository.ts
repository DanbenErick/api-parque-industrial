...
export class UsuarioRepository {
  ...
  `;
    ...
    SELECT u.id, u.documento_identidad, u.nombre_razonsocial, u.cargo_representante, 
           ...
           (
             SELECT CONCAT('[', GROUP_CONCAT(JSON_OBJECT('id', id, 'num_serie', num_serie, 'tipo', tipo, 'direccion', direccion, 'lectura_inicial', lectura_inicial, 'lectura_inicial_punta', lectura_inicial_punta, 'demanda_maxima_fuera_punta', demanda_maxima_fuera_punta, 'demanda_maxima_punta', demanda_maxima_punta) SEPARATOR ','), ']')
             FROM medidor 
