...
const generarMasivamente = async (periodo_id, admin_id) => {
  const connection = await db.getConnection();
  await connection.beginTransaction();

  try {
    // 1. Obtener datos del periodo
    const [periodos] = await connection.query('SELECT mes_anio, tarifa_kwh, tarifa_kwh_punta, tarifa_mantenimiento_normal, tarifa_mantenimiento_tiempo_real, factor_multiplicador FROM periodo_facturacion WHERE id = ?', [periodo_id]);
    ...
    // 1.5 Obtener configuración global para cobros automáticos (instalación)
    const [configs] = await connection.query('SELECT monto_instalacion_base FROM configuracion LIMIT 1');
    const config = configs[0] || { monto_instalacion_base: 0 };
    const instalacion_base = parseFloat(config.monto_instalacion_base) || 0;

    // 2. Obtener lecturas del periodo con sus usuarios correspondientes
    ...
    if (lecturas.length === 0) {
      // No hay lecturas, pero continuaremos para procesar a los que no tienen medidor
    }

    // 2.5 Obtener usuarios activos que no tienen medidor asignado
    const [usuariosSinMedidor] = await connection.query(`
    ...
    // 2.7 Filtrar usuarios que ya tienen un recibo generado para este periodo
    const [recibosExistentes] = await connection.query("SELECT usuario_id FROM recibo WHERE periodo_id = ? AND deleted_at IS NULL AND estado != 'Anulado'", [periodo_id]);
    const usuariosConRecibo = new Set(recibosExistentes.map(r => r.usuario_id));
    const lecturasAProcesar = lecturas.filter(l => !usuariosConRecibo.has(l.usuario_id));
    const usuariosSinMedidorAProcesar = usuariosSinMedidor.filter(u => !usuariosConRecibo.has(u.usuario_id));

    // 2.8 Cargar saldos a favor de todos los usuarios
    const [saldos] = await connection.query('SELECT id, saldo_a_favor FROM usuario WHERE saldo_a_favor > 0 AND deleted_at IS NULL');
    const saldoMap = {};
    saldos.forEach(s => { saldoMap[s.id] = parseFloat(s.saldo_a_favor) || 0; });
    const originalSaldoMap = { ...saldoMap };

    // 3. Obtener recibos previos para saber si hay deudas pendientes
    // Solo se suman los recibos Pendientes de meses pasados del mismo usuario.
    ...
    const deudaMap = {};
    deudas.forEach(d => { deudaMap[d.usuario_id] = parseFloat(d.deuda_total) || 0; });

    let procesados = 0;
    
    // Obtener último número de comprobante para este año
    ...
    for (const lectura of lecturasAProcesar) {
      ...
      if (lectura.tipo === 'Tiempo Real') {
        ...
        // The user manually inputs the rate (precio_factor_potencia) and the consumption (factor_potencia) in the frontend.
      ...
      // Arrastrar deuda del mes anterior
      const deuda_vencida = deudaMap[lectura.usuario_id] || 0;

      // Verificar si es cliente nuevo para cobrar instalación automáticamente (Opción C)
      ...
      const igv = 0; // Por defecto IGV 0 en esta lógica de negocio
      ...
      // Numero Comprobante secuencial sin colisiones
    ...
    // Procesar usuarios sin medidor (cargo fijo de S/ 10)
    ...
    // 7. Actualizar saldos a favor utilizados
...
const findByIdCompleto = async (id) => {
  const [rows] = await db.query(`
    SELECT r.id, r.numero_comprobante, r.cargo_energia, r.cargo_energia_punta, r.cargo_factor_potencia, r.cargo_mantenimiento,
           r.cargo_fijo, r.cargo_corte, r.multa_manipulacion, r.multa_reconexion, r.instalacion_medidor,
           r.deuda_pendiente, r.deuda_consumo, r.deuda_vencida, r.descuento, r.motivo_descuento,
  ...
  const recibo = rows[0];
  const [cargos_dinamicos] = await db.query('SELECT * FROM recibo_cargo_dinamico WHERE recibo_id = ? ORDER BY fecha_aplicacion ASC', [id]);
  recibo.cargos_dinamicos = cargos_dinamicos;
  
  return recibo;
};

const findHistorialConsumo = async (usuarioId, limit = 6, fechaMaxima = null) => {
  let query = `
...
const findHistorialConsumoMultiple = async (usuarioIds, limit = 7) => {
  if (!usuarioIds || usuarioIds.length === 0) return {};
  
  // Create a placeholder string for the IN clause
  const placeholders = usuarioIds.map(() => '?').join(',');
  
  // Using ROW_NUMBER to get the top N readings per user
  const query = `
  ...
  // Reverse each array to match the original behavior (oldest to newest)
...
const findAllCompletos = async (filters = {}) => {
  const { year, periodo, estado, search } = filters;
...
const generarIndividual = async (periodo_id, usuario_id, admin_id) => {
  const connection = await db.getConnection();
  await connection.beginTransaction();
  ...
  try {
    ...
    // 2. Verificar si el usuario ya tiene un recibo para este periodo
    const [recibosPrevios] = await connection.query(`
    ...
    // 3. Verificar si el usuario tiene medidor
    const [medidores] = await connection.query('SELECT id, cobro_instalacion_pendiente FROM medidor WHERE usuario_id = ? AND deleted_at IS NULL', [usuario_id]);
    const tieneMedidor = medidores.length > 0;
    
    let lectura = null;

    if (tieneMedidor) {
      // Obtener lectura del usuario para este periodo
      const [lecturas] = await connection.query(`
        SELECT l.id as lectura_id, l.consumo_calculado, l.consumo_calculado_punta, l.factor_potencia, l.precio_factor_potencia, m.usuario_id, m.id as medidor_id, m.cobro_instalacion_pendiente, m.tipo
        FROM lectura l
        INNER JOIN medidor m ON l.medidor_id = m.id
    ...
    // 3.5 Obtener saldo a favor actual
    const [usuarios] = await connection.query('SELECT saldo_a_favor FROM usuario WHERE id = ?', [usuario_id]);
    let saldo_a_favor = parseFloat(usuarios[0]?.saldo_a_favor || 0);
    const original_saldo = saldo_a_favor;

    // 4. Obtener deuda pasada del usuario
    const [deudas] = await connection.query(`
    ...
    // 7. Actualizar saldo a favor en usuario si fue consumido
...
module.exports = {
  ...
  generarIndividual,
  findByIdCompleto,
  findHistorialConsumo,
  findHistorialConsumoMultiple,
  findAllCompletos,
  findAllSinMedidor
};

const updateCargos = async (id, cargos) => {
  ...
  try {
    ...
    const [rows] = await connection.query('SELECT cargo_energia, cargo_mantenimiento FROM recibo WHERE id = ?', [id]);
    ...
    if (cargos_dinamicos !== undefined) {
      ...
      if (cargos_dinamicos.length > 0) {
        const values = cargos_dinamicos.map(c => [id, c.descripcion, c.tipo, c.monto]);
        await connection.query(`
...
const anularRecibo = async (id, motivo, admin_id) => {
  const connection = await db.getConnection();
  try {
...
module.exports.anularRecibo = anularRecibo;
