...
export class AuthRepository {
    ...
    public findUsuarioByDocumento = async (documento_identidad: string): Promise<IUsuario | null> => {
    ...
    public updateUltimoAcceso = async (id: number): Promise<void> => {
          await this.db.query(
            'UPDATE usuario SET ultimo_acceso = CURRENT_TIMESTAMP WHERE id = ?',
            [id]
          );
        };
    public registrarSesion = async (usuario_id: number, ip_address: string | null, user_agent: string | null): Promise<void> => {
          await this.db.query(
            'INSERT INTO auditoria_sesiones (usuario_id, ip_address, user_agent) VALUES (?, ?, ?)',
            [usuario_id, ip_address, user_agent]
          );
        };
    public getHistorialSesiones = async (page: number = 1, limit: number = 50): Promise<any[]> => {
    ...
    public getPasswordHashById = async (id: number): Promise<string | null> => {
          const [rows]: any = await this.db.query(
    ...
    public updatePassword = async (id: number, hashedPassword: string): Promise<number> => {
