const reciboRepo = require('../repositories/reciboRepository');
...
const getRecibosStats = async (req, res) => {
  try {
    const filters = {
      year: req.query.year,
      periodo: req.query.periodo,
...
const getRecibosByUsuario = async (req, res) => {
...
const generarRecibos = async (req, res) => {
...
const generarReciboIndividual = async (req, res) => {
...
const refacturarRecibo = async (req, res) => {
  const { id } = req.params;
...
// -------------------------------------------------------
const exportReciboPdf = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    doc.text('ANEXO 8 - JICAMARCA', rightCol + 40, y + 20);
      ...
      doc.fillColor('#ffffff').fontSize(10).font('Lexend-Bold').text('SUMINISTRO', margin, y + 10, { width: 90, align: 'center', lineBreak: false });
      doc.fillColor(PRIMARY).fontSize(16).font('Lexend-Bold').text(recibo.id.toString(), margin + 90, y + 8, { width: 70, align: 'center', lineBreak: false });
    ...
    doc.fillColor(PRIMARY).fontSize(9).font('Lexend-Bold').text('DETALLES DEL CONSUMO', margin, y + 5, { width: colWidth, align: 'center', lineBreak: false });
    y += 22;

    const drawConsumoItem = (label, val, dateStr, isBold = false) => {
      doc.fillColor(TEXT).fontSize(9).font(isBold ? 'Lexend-Bold' : 'Lexend');
      doc.text(label, margin + 5, y);
      doc.text(val, margin + 90, y, { width: 50, align: 'right', lineBreak: false });
      if(dateStr) doc.fillColor(TEXT).text(dateStr, margin + 145, y, { width: 85, align: 'right', lineBreak: false });
    ...
    // Historial
    doc.roundedRect(margin, y, colWidth, 18, 4).fill(HEADER_BG);
    doc.fillColor(PRIMARY).fontSize(9).font('Lexend-Bold').text('HISTORIAL DE CONSUMO', margin, y + 5, { width: colWidth, align: 'center', lineBreak: false });
    y += 22;
    ...
    if (historial.length > 0) {
      ...
      const cH = 75;
      const cy = y + 15;
      const maxC = Math.max(...historial.map(h => parseFloat(h.consumo_calculado) || 0), 10);
      ...
      historial.forEach((h, i) => {
        ...
        doc.fillColor(TEXT).fontSize(6).font('Lexend-Bold').text(Math.round(val).toString(), bX - 5, bY - 10, { width: barW + 10, align: 'center', lineBreak: false });
        ...
        doc.text(monthNames[mLabel] || mLabel, bX - 5, cy + cH + 5, { width: barW + 10, align: 'center', lineBreak: false });
      });
    } else {
      doc.fillColor(MUTED).fontSize(9).font('Lexend').text('No hay datos históricos.', margin, y + 50, { width: colWidth, align: 'center' });
    }
    y += 135;

    // Comunicado
    doc.roundedRect(margin, y, colWidth, 18, 4).fill(HEADER_BG);
    doc.fillColor(PRIMARY).fontSize(9).font('Lexend-Bold').text('COMUNICADO', margin, y + 5, { width: colWidth, align: 'center', lineBreak: false });
    y += 22;
    ...
    doc.roundedRect(margin, y, colWidth, 18, 4).fill(HEADER_BG);
    doc.fillColor(PRIMARY).fontSize(9).font('Lexend-Bold').text('FECHAS IMPORTANTES', margin, y + 5, { width: colWidth, align: 'center', lineBreak: false });
    y += 22;
    ...
    doc.fillColor(TEXT).fontSize(12).font('Lexend-Bold').text(formatDate(recibo.fecha_emision), margin, y + 6, { width: colWidth, align: 'center', lineBreak: false });
    y += 32;
    ...
    // Pago
    doc.fillColor(RED).fontSize(8).font('Lexend-Medium').text('ÚLTIMO DÍA DE PAGO', margin, y + 10, { width: colWidth / 2, align: 'center', lineBreak: false });
    doc.fillColor(RED).fontSize(14).font('Lexend-Bold').text(formatDate(recibo.fecha_vencimiento), margin, y + 22, { width: colWidth / 2, align: 'center', lineBreak: false });
    
    // Corte
    doc.fillColor(RED).fontSize(8).font('Lexend-Medium').text('FECHA DE CORTE', margin + colWidth / 2, y + 10, { width: colWidth / 2, align: 'center', lineBreak: false });
    doc.fillColor(RED).fontSize(14).font('Lexend-Bold').text(formatDate(fechaCorte), margin + colWidth / 2, y + 22, { width: colWidth / 2, align: 'center', lineBreak: false });
    ...
    doc.fillColor(PRIMARY).fontSize(8).font('Lexend-Bold').text('DETALLES DE LOS IMPORTES FACTURADOS', rightCol, ry + 5, { width: colWidth, align: 'center', lineBreak: false });
    ry += 25;
    ...
    doc.fillColor(RED).fontSize(10).font('Lexend-Bold').text(mes, rightCol + 100, ry + 5, { width: 50, align: 'center', lineBreak: false });
    doc.fillColor(TEXT).text(anio, rightCol + 150, ry + 5, { width: colWidth - 150, align: 'center', lineBreak: false });
    ry += 30;
    ...
    const drawRow = (desc, pu, monto, isBold = false) => {
      doc.fillColor(isBold ? PRIMARY : TEXT).fontSize(9).font(isBold ? 'Lexend-Bold' : 'Lexend');
      doc.text(desc, rightCol + 5, ry, { lineBreak: false });
      if(pu !== null) doc.text(formatNum(pu), rightCol + 100, ry, { width: 45, align: 'right', lineBreak: false });
      doc.text(formatNum(monto), rightCol + 160, ry, { width: 65, align: 'right', lineBreak: false });
      ry += 16;
    };
    ...
    if (descuento > 0) {
      ...
      doc.text('-' + formatNum(descuento), rightCol + 160, ry, { width: 65, align: 'right', lineBreak: false });
      ry += 16;
    }
    ...
    doc.text(formatNum(total), rightCol + 160, ry, { width: 65, align: 'right', lineBreak: false });
    ry += 30;
      ...
      doc.fillColor(PRIMARY).fontSize(16).font('Lexend-Bold').text(formatNum(total), rightCol + colWidth - amtWidth - 8, ry + 14, { width: amtWidth, align: 'center', lineBreak: false });
    ...
    doc.text('Conserve su voucher', margin, finalY + 15, { width: contentWidth - 20, align: 'right' });
    doc.text('como comprobante.', margin, finalY + 27, { width: contentWidth - 20, align: 'right' });
...
const drawReciboLayoutV2 = (doc, recibo, historial, logoPath) => {
  ...
  doc.fillColor('#fff').fontSize(10).font('Lexend-Bold').text('DETALLE DE CONSUMO', margin, y + 8, { width: colLeftW, align: 'center' });

  let cy = y + 35;
  doc.fillColor(TEXT_MAIN).fontSize(9).font('Lexend');
  doc.text('Lectura Actual:', margin + 12, cy);
  doc.font('Lexend-Bold').text(formatNum(recibo.lectura_actual), margin + 90, cy, { width: 55, align: 'right' });
  doc.font('Lexend').text(formatDate(recibo.periodo_fin), margin + 155, cy);
  
  cy += 20;
  doc.text('Lectura Anterior:', margin + 12, cy);
  doc.font('Lexend-Bold').text(formatNum(recibo.lectura_anterior), margin + 90, cy, { width: 55, align: 'right' });
  ...
  doc.text(formatNum(recibo.consumo_calculado), margin + 90, cy + 3, { width: 55, align: 'right' });
  ...
  doc.fillColor('#fff').fontSize(10).font('Lexend-Bold').text('HISTORIAL DE CONSUMO', margin, gy + 8, { width: colLeftW, align: 'center' });
  ...
  if (historial.length > 0) {
    ...
    doc.fillColor(TEXT_MAIN).fontSize(7).font('Lexend-Medium').text(Math.round(val).toString(), bX - 5, bY - bH - 12, { width: barW + 10, align: 'center' });
    
    const mLabel = h.mes_anio ? h.mes_anio.substring(5) : '';
    const monthNames = { '01':'Ene','02':'Feb','03':'Mar','04':'Abr','05':'May','06':'Jun','07':'Jul','08':'Ago','09':'Sep','10':'Oct','11':'Nov','12':'Dic' };
    doc.text(monthNames[mLabel] || mLabel, bX - 5, bY + 8, { width: barW + 10, align: 'center' });
    });
  }
  ...
  doc.text('P.U.', colRightX + 170, y + 8, { width: 40, align: 'right' });
  doc.text('MONTO (S/)', colRightX + 220, y + 8, { width: 70, align: 'right' });

  let ry = y + 35;
  let isZebra = false;

  const drawRowV2 = (desc, pu, monto, isBold = false, isIndented = false) => {
    if (isZebra) {
    ...
    if(pu !== null) doc.text(formatNum(pu), colRightX + 170, ry, { width: 40, align: 'right' });
    doc.text(formatNum(monto), colRightX + 220, ry, { width: 70, align: 'right' });
    ry += 22;
    isZebra = !isZebra;
  };
  ...
  if (descuento > 0) {
    ...
    doc.text('-' + formatNum(descuento), colRightX + 220, ry, { width: 70, align: 'right' });
    ry += 22;
    isZebra = !isZebra;
  }
  ...
  doc.fontSize(22).text(formatNum(total), colRightX + 130, ry + 15, { width: 160, align: 'right' });
  ...
  doc.font('Lexend-Bold').fontSize(14).text(recibo.id.toString(), doc.page.width - margin - 80, talonY + 42, { width: 80, align: 'right' });
  
  doc.font('Lexend-Medium').fontSize(9).text('Monto a Pagar S/:', doc.page.width - margin - 220, talonY + 70);
  doc.font('Lexend-Bold').fontSize(18).text(formatNum(total), doc.page.width - margin - 90, talonY + 65, { width: 90, align: 'right' });
};
...
// -------------------------------------------------------
const drawReciboLayoutV3 = (doc, recibo, historial, logoPath) => {
  // ── PALETA ──
  const NAVY      = '#0f172a';
  const TEAL      = '#0d9488';
  ...
  doc.text('RECIBO DE LUZ', badgeX, 20, { width: badgeW, align: 'center' });
  doc.fillColor(WHITE).fontSize(16).font('Lexend-Bold');
  doc.text(recibo.numero_comprobante || `REC-${recibo.id}`, badgeX, 32, { width: badgeW, align: 'center' });
  ...
  doc.text('TARIFA', margin + W * 0.55, y + 9, { width: 60, align: 'right', lineBreak: false });
  doc.text('IMPORTE (S/)', margin + W * 0.72, y + 9, { width: W * 0.25, align: 'right', lineBreak: false });
  y += 30;
  ...
  if (recibo.cargos_dinamicos && Array.isArray(recibo.cargos_dinamicos)) {
    recibo.cargos_dinamicos.forEach(cd => {
  ...
  items.forEach((item, i) => {
    ...
    if (item.tarifa) {
      ...
      doc.text(item.tarifa, margin + W * 0.48, y + 6, { width: 80, align: 'right', lineBreak: false });
    }
    doc.fillColor(NAVY).fontSize(9).font('Lexend-Bold');
    doc.text(fmt(item.monto), margin + W * 0.72, y + 5, { width: W * 0.25, align: 'right', lineBreak: false });
    y += rowH;
  });
  ...
  doc.text(fmt(subtotalMes), margin + W * 0.72, y + 2, { width: W * 0.25, align: 'right', lineBreak: false });
  y += 18;
  ...
  if (deudaVencida > 0) {
    ...
    doc.text(fmt(deudaVencida), margin + W * 0.72, y + 2, { width: W * 0.25, align: 'right', lineBreak: false });
    y += 18;
  }
  ...
  if (descuento > 0) {
    ...
    doc.font('Lexend-Bold');
    doc.text('-' + fmt(descuento), margin + W * 0.72, y + 2, { width: W * 0.25, align: 'right', lineBreak: false });
    y += 18;
  }
  ...
  doc.text('TOTAL A PAGAR', margin + W * 0.4, y + 4, { width: W * 0.3, align: 'right', lineBreak: false });
  
  doc.fillColor(NAVY).fontSize(20).font('Lexend-Bold');
  doc.text(`S/ ${fmt(total)}`, margin + W * 0.72, y, { width: W * 0.25, align: 'right', lineBreak: false });
  ...
  if (historial.length > 0) {
    ...
    const maxVal = Math.max(...historial.map(h => parseFloat(h.consumo_calculado) || 0), 10);
    ...
    historial.forEach((h, i) => {
      ...
      doc.text(Math.round(val).toString(), bx - 4, by - 10, { width: barW + 8, align: 'center', lineBreak: false });

      const mLabel = h.mes_anio ? h.mes_anio.substring(5) : '';
      const monthNames = { '01':'Ene','02':'Feb','03':'Mar','04':'Abr','05':'May','06':'Jun','07':'Jul','08':'Ago','09':'Sep','10':'Oct','11':'Nov','12':'Dic' };
      doc.fillColor(SLATE_LT).fontSize(5).font('Lexend');
      doc.text(monthNames[mLabel] || mLabel, bx - 4, chartY + chartH + 4, { width: barW + 8, align: 'center', lineBreak: false });
    });
  } else {
...
const exportReciboPdfV3 = async (req, res) => {
  const { id } = req.params;
...
const exportAllRecibosPdfV2 = async (req, res) => {
  try {
    ...
    const recibos = await reciboRepo.findAllCompletos(filters);
    ...
    const usuarioIds = [...new Set(recibos.map(r => r.usuario_id))];
...
const exportReporteDeudasExcel = async (req, res) => {
  try {
    ...
    const recibos = await reciboRepo.findAllCompletos(filtro);
    ...
    const recibosDeudaIds = recibosDeuda.map(r => r.id);
    let cargosDinamicosByRecibo = {};
    if (recibosDeudaIds.length > 0) {
      const [allDinamicos] = await db.query('SELECT * FROM recibo_cargo_dinamico WHERE recibo_id IN (?)', [recibosDeudaIds]);
      allDinamicos.forEach(cd => {
    ...
    for (const socioId in gruposSocios) {
      ...
      grupo.recibos.sort((a, b) => new Date(b.fecha_emision) - new Date(a.fecha_emision));
      ...
      const recibosCronologicos = [...grupo.recibos].sort((a, b) => new Date(a.fecha_emision) - new Date(b.fecha_emision));

      recibosCronologicos.forEach(r => {
        let lineasDetalle = [];
        ...
        const dinamicos = cargosDinamicosByRecibo[r.id] || [];
        dinamicos.forEach(cd => {
...
module.exports = {
  ...
  getRecibosByUsuario,
  generarRecibos,
  generarReciboIndividual,
  exportReciboPdf,
  exportReciboPdfV2,
  exportReciboPdfV3,
  exportReporteExcel,
  exportReporteDeudasExcel,
  getReciboById
};

const updateCargos = async (req, res) => {
  const { id } = req.params;
